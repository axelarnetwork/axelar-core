...
func TestKeeper_AssignNextMasterKey_RotateMasterKey_NewKeyIsSet(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		...
		assert.NoError(t, s.Keeper.AssignNextMasterKey(ctx, chain, snapshotHeight, expectedKey.ID))
...
func TestKeeper_AssignNextMasterKey_RotateMasterKey_MultipleTimes_PreviousKeysStillAvailable(t *testing.T) {
	for i := 0; i < 100; i++ {
		...
		for i := range masterKeys {
			...
			key := s.SetKey(t, ctx)
			masterKeys[i] = key.Value

			assert.NoError(t, s.Keeper.AssignNextMasterKey(ctx, chain, snapshotHeight, key.ID))
