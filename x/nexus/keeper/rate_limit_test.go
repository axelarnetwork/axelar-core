...
func TestRateLimitTransfer(t *testing.T) {
	...
	givenKeeper.
		...
		Then("rate limit transfer succeeds", func(t *testing.T) {
			err := k.RateLimitTransfer(ctx, chain, asset, direction, ctx.Logger())
			assert.NoError(t, err)
		}).
		Run(t, repeated)
	...
	givenKeeper.
		...
		Then("rate limit transfer succeeds", func(t *testing.T) {
			err := k.RateLimitTransfer(ctx, chain, asset, exported.TransferDirectionFrom, ctx.Logger())
			assert.NoError(t, err)

			err = k.RateLimitTransfer(ctx, chain, asset, exported.TransferDirectionTo, ctx.Logger())
			assert.NoError(t, err)
		}).
