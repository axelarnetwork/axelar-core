...
func TestTransfer(t *testing.T) {
	...
	whenAssetIsRegisteredOnSource := When("asset is registered on source chain", func() {
		funcs.MustNoErr(k.RegisterAsset(ctx, source, nexus.Asset{Denom: asset, IsNativeAsset: true}, sdk.ZeroUint()))
	})
...
func setup(cfg params.EncodingConfig) (nexusKeeper.Keeper, sdk.Context) {
	...
	for _, chain := range chains {
		...
		for _, asset := range assets {
			...
			if err := k.RegisterAsset(ctx, chain, nexus.NewAsset(asset, isNative), sdk.ZeroUint()); err != nil {
				panic(err)
			}
