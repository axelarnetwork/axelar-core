...
func getStakingKeeper(cdc *codec.Codec) staking.Keeper {
	...
	keys := sdk.NewKVStoreKeys(staking.StoreKey, supply.StoreKey, auth.StoreKey, params.StoreKey)
...
func NewTestKeeper(validators ...typesStaking.Validator) types.StakingKeeper {
	...
	for _, val := range validators {
		...
		mock.validators = append(mock.validators, val)
		mock.totalPower = mock.totalPower.AddRaw(val.GetConsensusPower())
	}
