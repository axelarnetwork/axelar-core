...
func NewHandler(k types.BTCKeeper, v types.Voter, signer types.Signer, n types.Nexus, snapshotter types.Snapshotter) sdk.Handler {
	...
	h := func(ctx sdk.Context, msg sdk.Msg) (*sdk.Result, error) {
		...
		case *types.LinkRequest:
			...
			if err == nil {
				result.Log = fmt.Sprintf("successfully linked deposit %s on chain %s to recipient %s on chain %s for asset %s", res.DepositAddr, exported.Bitcoin.Name, msg.RecipientAddr, msg.RecipientChain, exported.NativeAsset)
			}
			return result, err
