...
func NewHandler(k types.BaseKeeper, n types.Nexus, b types.BankKeeper, t types.IBCTransferKeeper, c types.ChannelKeeper, a types.AccountKeeper) sdk.Handler {
	...
	h := func(ctx sdk.Context, msg sdk.Msg) (*sdk.Result, error) {
		...
		case *types.LinkRequest:
			...
			if err == nil {
				result.Log = fmt.Sprintf("successfully linked deposit %s on %s to recipient %s on recipient chain %s for asset '%s'", res.DepositAddr, exported.Axelarnet.Name, msg.RecipientAddr, msg.RecipientChain, msg.Asset)
			}
			return result, err
