...
func (i IBCKeeper) SendGeneralMessage(c context.Context, destChain nexus.ChainName, destAddr string, asset sdk.Coin, memo string, id nexus.MessageID) error {
	...
	path, ok := i.GetIBCPath(ctx, destChain)
	if !ok {
		return fmt.Errorf("%s does not have a valid IBC path", destChain)
	}

	pathSplit := strings.SplitN(path, "/", 2)
	if len(pathSplit) != 2 {
		return fmt.Errorf("invalid path %s for chain %s", path, destChain)
	}
