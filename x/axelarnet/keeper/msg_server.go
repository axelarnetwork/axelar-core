...
func (s msgServer) CallContract(c context.Context, req *types.CallContractRequest) (*types.CallContractResponse, error) {
	...
	if req.Fee != nil {
		normalizedCoin, err := s.nexus.NewLockableAsset(ctx, s.ibcK, s.bank, req.Fee.Amount)
		if err != nil {
		...
		feePaidEvent := types.FeePaid{
			...
			Fee:       req.Fee.Amount,
			Asset:     normalizedCoin.GetAsset().Denom,
		}
...
func (s msgServer) ConfirmDeposit(c context.Context, req *types.ConfirmDepositRequest) (*types.ConfirmDepositResponse, error) {
	...
	normalizedCoin, err := s.nexus.NewLockableAsset(ctx, s.ibcK, s.bank, coin)
	if err != nil {
		return nil, err
	}
	...
	transferID, err := s.nexus.EnqueueForTransfer(ctx, depositAddr, normalizedCoin.GetAsset())
	if err != nil {
		return nil, err
	}
...
func (s msgServer) RouteIBCTransfers(c context.Context, _ *types.RouteIBCTransfersRequest) (*types.RouteIBCTransfersResponse, error) {
	...
	for _, c := range s.GetCosmosChains(ctx) {
		...
		for _, p := range pendingTransfers {
			coin, err := s.nexus.NewLockableAsset(ctx, s.ibcK, s.bank, p.Asset)
			if err != nil {
			...
			funcs.MustNoErr(s.EnqueueIBCTransfer(ctx, types.NewIBCTransfer(types.AxelarIBCAccount, p.Recipient.Address, coin.GetCoin(ctx), portID, channelID, p.ID)))
...
func transfer(ctx sdk.Context, k Keeper, n types.Nexus, b types.BankKeeper, ibc types.IBCKeeper, recipient sdk.AccAddress, coin sdk.Coin) error {
	c, err := n.NewLockableAsset(ctx, ibc, b, coin)
	if err != nil {
		return err
	}
