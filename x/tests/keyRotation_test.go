...
func TestBitcoinKeyRotation(t *testing.T) {
	...
	// start new keygen
	masterKeyID := randStrings.Next()
	keygenResult := <-chain.Submit(tssTypes.NewStartKeygenRequest(randomSender(), masterKeyID, 0, tss.WeightedByStake))
	...
	assignKeyResult := <-chain.Submit(tssTypes.NewAssignKeyRequest(randomSender(), btc.Bitcoin.Name, masterKeyID, tss.MasterKey))
	...
	rotateResult := <-chain.Submit(tssTypes.NewRotateKeyRequest(randomSender(), btc.Bitcoin.Name, tss.MasterKey))
