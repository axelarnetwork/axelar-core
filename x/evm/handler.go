...
func NewHandler(k types.BaseKeeper, t types.TSS, v types.Voter, s types.Signer, n types.Nexus, snapshotter types.Snapshotter) sdk.Handler {
	...
	h := func(ctx sdk.Context, msg sdk.Msg) (*sdk.Result, error) {
		...
		case *types.LinkRequest:
			...
			if err == nil {
				result.Log = fmt.Sprintf("successfully linked deposit %s on chain %s to recipient address %s on chain %s for asset '%s'", res.DepositAddr, msg.Chain, msg.RecipientAddr, msg.RecipientChain, msg.Asset)
			}
			return result, err
