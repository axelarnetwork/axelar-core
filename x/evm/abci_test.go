...
func TestHandleGeneralMessage(t *testing.T) {
	...
	givenMessage := Given("a message", func() {
		msg = nexus.NewGeneralMessage(evmTestUtils.RandomHash().Hex(), sender, receiver, evmCrypto.Keccak256(payload), nexus.Approved, nil, evmTestUtils.RandomHash().Bytes()[:])
		...
		return When("having general messages", func() {
			n.GetSentMessagesFunc = func(_ sdk.Context, chain nexus.ChainName, limit int64) []nexus.GeneralMessage {
				...
				for i := 0; i < int(limit) && i < numPerChain[chain]; i++ {
					...
					msg := nexus.NewGeneralMessage(evmTestUtils.RandomHash().Hex(), sender, receiver, evmTestUtils.RandomHash().Bytes(), nexus.Sent, nil, evmTestUtils.RandomHash().Bytes()[:])
