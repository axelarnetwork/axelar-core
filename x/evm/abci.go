...
func EndBlocker(ctx sdk.Context, req abci.RequestEndBlock, k types.EVMKeeper) []abci.ValidatorUpdate {
	...
	for _, event := range ctx.EventManager().ABCIEvents() {
		...
		if event.Type == tsstypes.EventTypeSign {
			...
			if pendingSignCommand, ok := k.GetPendingSignCommand(ctx, sigID); ok {
				...
				ctx.EventManager().EmitEvent(sdk.NewEvent(types.EventTypeSignedCommandID,
					...
					sdk.NewAttribute(types.AttributeKeyCommandSelector, pendingSignCommand.CommandSelector),
					sdk.NewAttribute(types.AttributeKeyChain, pendingSignCommand.Chain),
					sdk.NewAttribute(tsstypes.AttributeKeyResult, result),
				))

				k.Logger(ctx).Info(fmt.Sprintf("%s evm gateway %s command %s is fully signed", pendingSignCommand.Chain, pendingSignCommand.CommandSelector, sigID))
			} else {
				continue
			}
		}
	...
	ctx.Logger().Info("evm blocker got %d events: %s", strings.Join(events, ","))
