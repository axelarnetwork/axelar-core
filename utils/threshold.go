...
func (m Threshold) IsMet(share math.Int, total math.Int) bool {
...
func (m Threshold) GT(t Threshold) bool {
	return math.NewInt(m.Numerator).MulRaw(t.Denominator).GT(math.NewInt(m.Denominator).MulRaw(t.Numerator))
}

// GTE returns true if and only if threshold is greater than or equal to the given one
func (m Threshold) GTE(t Threshold) bool {
	return math.NewInt(m.Numerator).MulRaw(t.Denominator).GTE(math.NewInt(m.Denominator).MulRaw(t.Numerator))
}

// LT returns true if and only if threshold is less than the given one
func (m Threshold) LT(t Threshold) bool {
	return math.NewInt(m.Numerator).MulRaw(t.Denominator).LT(math.NewInt(m.Denominator).MulRaw(t.Numerator))
}

// LTE returns true if and only if threshold is less than or equal to the given one
func (m Threshold) LTE(t Threshold) bool {
	return math.NewInt(m.Numerator).MulRaw(t.Denominator).LTE(math.NewInt(m.Denominator).MulRaw(t.Numerator))
}
