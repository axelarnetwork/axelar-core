...
func setup(t *testing.T) (sdk.Context, *codec.ProtoCodec) {
...
func TestNewBlockHeightKVQueue(t *testing.T) {
	...
	whenHavingASequentialKVQueue := When("a sequential kv queue", func() {
		ctx, cdc := setup(t)
		store := NewNormalizedStore(ctx.KVStore(store.NewKVStoreKey(stringGen.Next())), cdc)
		...
		kvQueue = NewGeneralKVQueue(rand.Str(5), store, log.NewTestLogger(t), func(value codec.ProtoMarshaler) Key {
	...
	t.Run("Enqueue and Dequeue", testutils.Func(func(t *testing.T) {
		...
		kvQueue := NewBlockHeightKVQueue("test-enqueue-dequeue", store, blockHeight, log.NewTestLogger(t))
	...
	t.Run("ImportState and ExportState", testutils.Func(func(t *testing.T) {
		...
		blockHeight := rand.I64Between(1, 10000)
		kvQueue := NewBlockHeightKVQueue("test-queue", store, blockHeight, log.NewTestLogger(t))
...
func TestNewSequenceKVQueue(t *testing.T) {
	...
	t.Run("enqueue within limit and dequeue", testutils.Func(func(t *testing.T) {
		...
		store := prefix.NewStore(ctx.KVStore(store.NewKVStoreKey(stringGen.Next())), []byte("test-enqueue-dequeue"))
		kvQueue := NewSequenceKVQueue(NewNormalizedStore(store, cdc), uint64(queueSize), log.NewTestLogger(t))
		for i := 0; i < int(itemCount); i++ {
			items[i] = rand.Str(10)
		}
