...
Predictable address example (also see '<appd> query wasm build-address -h'):
$ <appd> tx wasm instantiate2 1 '{"foo":"bar"}' $(echo -n "testing" | xxd -ps) --admin="$(<appd> keys show mykey -a)" \
