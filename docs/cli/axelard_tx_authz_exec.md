...
execute tx on behalf of granter account:
Example:
$ axelard tx authz exec tx.json --from grantee
$ axelard tx bank send <granter> <recipient> --from <granter> --chain-id <chain-id> --generate-only > tx.json && axelard tx authz exec tx.json --from grantee

```
axelard tx authz exec [tx-json-file] --from [grantee] [flags]
```
